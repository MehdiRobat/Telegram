...
async def admin_text_router(client: Client, message: Message):
    ...
    if uid in schedule_data:
        ...
        # اگر چیز دیگری بود، نادیده می‌گیریم تا state بعداً پاک شود
        return
    ...
    if uid in upload_data:
        ...
        if step == "awaiting_caption":
            ...
            if "pending_file_id" not in data:
                # اگر کاربر به‌جای فایل اول کپشن داد، به مرحله‌ی دریافت فایل برگرد
...
# ---------------------- انتشار فوری به کانال (فقط 1 پست: کاور+کپشن+دکمه) ----------------------
...
async def film_pub_go_cb(client: Client, cq: CallbackQuery):
    """ارسال فوری یک پست کانالی: فقط کاور + کپشن + دکمه دانلود (دیپ‌لینک)"""
    await cq.answer()
    ...
    kb = build_deeplink_kb(film_id)

    try:
        if film.get("cover_id"):
            await client.send_photo(channel_id, photo=film["cover_id"], caption=caption, reply_markup=kb)
        else:
            # اگر کاور نداریم، پیام متنی + دکمه بفرست
            await client.send_message(channel_id, text=caption, reply_markup=kb)
    except Exception as e:
        return await cq.message.edit_text(f"❌ خطا در ارسال: {e}")

    await cq.message.edit_text("✅ پست ارسال شد (کاور + دکمه دانلود).")
...
async def send_scheduled_posts():
    ...
    هر یک دقیقه اجرا می‌شود:
      ...
      - دقیقاً یک پست کانالی می‌فرستد: کاور + کپشن + دکمه دانلود
    ...
    for post in posts:
        ...
        caption = compose_channel_caption(film)
        kb = build_deeplink_kb(film["film_id"])

        try:
            if film.get("cover_id"):
                await bot.send_photo(post["channel_id"], photo=film["cover_id"], caption=caption, reply_markup=kb)
            else:
                await bot.send_message(post["channel_id"], text=caption, reply_markup=kb)
        except Exception as e:
